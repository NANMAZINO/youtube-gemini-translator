...
이제 `src/`가 메인 확장 런타임이며, `npm run build` 결과물인 `dist/`를 Chrome에 로드하면 됩니다. 남아 있는 `extension/` 트리는 정리 작업 중 비교용으로만 유지하는 레거시 참고 경로입니다.
...
- `docs/rebuild/`: 마이그레이션 계획, 아키텍처 스냅샷, 진행 기록
- `extension/`: 정리 중 비교용으로 남겨둔 레거시 런타임 참고 경로
...
- 남아 있는 `extension/` 트리는 정리 과정에서 동작 비교를 위한 참고 자료일 뿐입니다.
