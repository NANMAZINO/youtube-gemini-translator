...
test('withRetry throws after the retry budget is exhausted', async () => {
  let attempts = 0;
...
test('withRetry normalizes non-Error throws', async () => {
  await assert.rejects(
    withRetry(
      async () => {
        throw 'boom';
...
test('withRetry throws AbortError immediately when the signal is already aborted', async () => {
...
test('withRetry aborts while waiting in backoff sleep', async () => {
  const controller = new AbortController();
  let attempts = 0;
