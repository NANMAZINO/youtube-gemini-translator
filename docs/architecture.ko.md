...
| 분류 | 키 | 비고 |
|---|---|---|
| **설정** | `targetLang`, `sourceLang`, `thinkingLevel`, `resumeMode`, `uiLocale`, `themeMode` | 번역 설정은 호환성을 유지하고, UI 선호도는 읽기 시 기본값으로 앞으로 보정 |
...
팝업의 UI 언어/테마 변경도 `settings.save`를 통해 저장한 뒤 `chrome.storage.onChanged`로 콘텐츠 런타임에 전파되어, YouTube 패널과 오버레이가 페이지 새로고침 없이 다시 렌더링됩니다.
...
| **팝업 레이아웃** | 사용량 요약은 간결하고 한눈에 — 안정적 2×2 통계 레이아웃을 유지하고, UI 언어/테마 컨트롤은 제목 아래 hero 행에서 버전 배지와 함께 배치 |
