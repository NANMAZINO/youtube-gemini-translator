...
The Vite build packages the default extension artifact into `dist/`, and `npm run dev` / `npm run build` now target the main `src/` runtime by default. The retained `extension/` tree is archived as a legacy regression reference while post-cutover cleanup removes obsolete slices.
